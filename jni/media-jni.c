...
JNIEXPORT jint JNICALL jni_um_vdec_setSurface(JNIEnv * env, jobject obj)
{
	...
	jclass surfaceClass = (*env)->FindClass(env, "android/view/Surface");	//!< get java class instance from system class
	...
	if(1){
		...
		fields.surface_native = (*env)->GetFieldID(env, surfaceClass, "mNativeSurface", "I");	//!< get the no of mNativeSurface var in the system
	}else{

		fields.surface_native = (*env)->GetFieldID(env, surfaceClass, "mNativeObject", "I");
	}
	...
	vout_android_java_surf = (*env)->NewGlobalRef(env, surface);
...
JNIEXPORT jint JNICALL jni_um_vdec_decode(JNIEnv * env, jobject obj, jbyteArray in, jint len)
{
	...
	jbyte* buffer = (jbyte*)(*env)->GetByteArrayElements(env, in, 0);
                                ...
                                JNINativeMethod* gMethods, int numMethods)
{
	...
	clazz = (*env)->FindClass(env, className);	//!< get java class instance from my java class
	...
	fields.surface = (*env)->GetFieldID(env, clazz, "mSurface", "Landroid/view/Surface;");	//!< get the class of surface var in the java class
